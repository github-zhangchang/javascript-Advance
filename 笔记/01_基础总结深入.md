...
## 2). 为什么要用函数?
    提高复用性
    便于阅读交流
## 3). 函数也是对象(一般函数对象、构造函数对象、如果证明函数也是对象？函数有自身的属性、方法，也可以添加属性、方法)
    fn instanceof Object===true
    函数有属性: prototype

    函数有方法: call()/apply()/bind()
    数组有方法：forEach()/map()/filter()
    对象有方法：toString()/hasOwnProperty()
...
## 5). 函数中的this
    ...
    ** bind() - 返回一个新的函数对象，并指定了第一个参数为 this
